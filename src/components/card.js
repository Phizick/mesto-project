const createNewCard = (cardName, cardLink) => {  
  ...
  const renderCard = (cardName, cardLink) => {galleryList.prepend(createNewCard(cardName, cardLink))};
  
  initialCards.forEach(item => {renderCard(item.name, item.link)});

  buttonOpenPopupCard.addEventListener("click", () => {openPopup(popupAddCard)});
